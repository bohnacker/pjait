...
//is a
	...
	// line(-7*xg , 1*yg , -6*xg  , 1*yg)

 // line(-7*xg , 1.50*yg , -6.5*xg  , 1.50*yg)
 // line(-6.5*xg , 2*yg , -7*xg  , 1.50*yg)


	// line(-7*xg , 2.50*yg , -6*xg  , 2.50*yg)
	// line(-7*xg , 3*yg , -6*xg  , 3*yg)

	// line(-7*xg , 3*yg , -7*xg , 2.50*yg)
	// line(-6.25*xg , 3*yg , -6.25*xg , 2.5*yg)
	// pop()
}

function line_quad(x1, y1, x2, y2, x3, y3, x4, y4){

	var lol=[x1, y1, x2, y2, x3, y3, x4, y4, x1, y1]

	for (var i = 0; i < lol.length; i=i+2) {
		
		line(lol[0+i],lol[1+i],lol[2+i],lol[3+i])
...
function draw_afro(){
	...
	line_quad(2*xg, 0, 0, 0, 0, -yg, 2*xg, -yg);
	line_quad(2*xg, -yg,         0, -yg,           0, -2*yg,  xg, -2*yg)
...
function draw() {
	...
	stroke((frameCount*2+i*20)%255, 255, 140);
